...
from toolbox.models.MSKANet_S. import MSKANet_S
...
if __name__ == "__main__":
    net = MSKANet_S().cuda()
    # net = GSGNet_T().cuda()
    net.load_state_dict(torch.load('./weight/MSKANet_S-Vaihingen-loss.pth'))
    # net.load_state_dict(torch.load('./weight/MSKANet_S-Potsdam-loss.pth'))

    _, all_preds, all_gts = test(net, test_ids, all=True, stride=48)
    # print(accuary)
    for p, id_ in zip(all_preds, test_ids):
        img = convert_to_color_(p)
        io.imsave('./results/Vaihingen/MSKANet_S/top_mosaic_09cm_area{}.png'.format(id_), img)
        # io.imsave('./results/Potsdam/MSKANet_S/top_mosaic_09cm_area{}.png'.format(id_), img)